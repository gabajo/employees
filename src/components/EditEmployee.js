...
export default function EditEmployee() {
  ...
  async function getOne() {
    ...
    setName(json.name);
    setSecondName(json.secondName);
    setAddresses(json.addresses);
    setBirthDate(json.birthDate);
    setPhoneNumbers(json.phoneNumbers);
    setPhoto(json.photo);
  }
  ...
  async function sendEmployee() {
    ...
    await fetch("http://localhost:8080/employees/" + id, options);
  }
  ...
  return (
    <div>
      <div>
        ...
        <div style={{ display: "flex" }}>
          <button onClick={sendEmployee}>Edit</button>
        </div>
      </div>
    </div>
  );
